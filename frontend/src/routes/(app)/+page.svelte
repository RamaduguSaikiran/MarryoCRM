      ...
      <div class="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-end">
        <div class="space-y-2">
          <div class="flex items-center gap-3">
            <div
              class="flex size-10 items-center justify-center rounded-[var(--radius-lg)] bg-gradient-to-br from-[var(--color-primary-default)] to-[var(--color-primary-active)] shadow-lg dark:shadow-[var(--color-primary-default)]/20"
            >
              <Sparkles class="size-5 text-white" />
            </div>
            <div>
              <p class="text-sm font-medium text-[var(--text-secondary)]">{getGreeting()}</p>
              <h1 class="text-2xl font-bold tracking-tight text-[var(--text-primary)] md:text-3xl marryo-heading">
                Dashboard
              </h1>
            </div>
          </div>
