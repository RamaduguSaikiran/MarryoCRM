...
>
  ...
  <div class="relative flex min-h-[4.5rem] items-center justify-between gap-6 px-6 py-4 md:px-8">
    ...
    <div class="flex flex-col gap-1">
      <div class="flex items-baseline gap-3">
        <h1
          class="text-foreground text-2xl font-bold tracking-tight md:text-[1.75rem] marryo-heading"
          style="font-family: 'Cormorant Garamond', Georgia, serif; letter-spacing: 0.01em;"
        >
          {title}
        </h1>
        {#if subtitle}
  ...
  <!-- Bottom accent line - warm gold gradient -->
  <div
    class="absolute right-0 bottom-0 left-0 h-px bg-gradient-to-r from-transparent via-[var(--color-gold-default,hsl(38,50%,55%))]/40 to-transparent"
  ></div>
</header>
