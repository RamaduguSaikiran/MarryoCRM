<script>
  ...
  /** @type {Props} */
  let { user = {}, org_name = 'MarryoCRM' } = $props();
            ...
            {#snippet child({ props })}
              ...
              >
                ...
                <div class="relative">
                  ...
                  {:else}
                    <div
                      class="ring-sidebar-border/50 flex size-8 items-center justify-center rounded-full bg-gradient-to-br from-[var(--color-primary-default)] to-[var(--color-primary-active)] text-xs font-semibold text-white ring-2 transition-all duration-150 group-hover/user:ring-[var(--color-primary-default)]/30"
                    >
...
<style>
  /* Refined sidebar typography — Inter (body) + Cormorant Garamond (brand) */
  ...
  /* Sidebar brand name uses serif */
  :global(.hubspot-sidebar .sidebar-brand-name) {
  ...
  /* Gold accent bar for active nav items */
  :global(.hubspot-sidebar .nav-item-active)::before {
    content: '';
    position: absolute;
    left: 0;
    top: 25%;
    ...
    background: var(--color-gold-default, hsl(38, 50%, 55%));
  }
</style>
